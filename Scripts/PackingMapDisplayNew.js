...
var removethelayer= function() {
    var found = null;
    var layers = map.getLayers();

     for (let index = 0; index < layers.a.length; index++) {
        if (String(layers.a[index].type).toUpperCase()=='VECTOR'){
            layers.a.splice(index,1);
            if(index != 0){index--;}
        }
...
var startVectorSource

var startVectorSource1
$.TimerDrawLineInit = function (data) {
    //layersCollection = map.getLayers();
   ...
   if (layerLines != null) {
    ...
    startVectorSource1=startVectorSource;
    ...
    datatmp.push(data);
    //datatmp.push(data);
    lineDrawList = new ol.geom.LineString(datatmp, 'XY');
   // lineDrawList = new ol.geom.LineString(data, 'XY');
    ...
    linefeature.setStyle(lineStyle[0]);
...
$.TimerDrawLineReset = function (array) {
    //removethelayer();
    // removethelayer();
    // removethelayer();getSource().clear()
    //startVectorSource1.getSource().clear();
    //map.removeLayer(map.vectorLayers);
    //map.getLayers().clear();
    // map.removeLayer();
    // map.removeLayer();
    ...
    // }
        ...
        // var flatCoordinates = geometry.getFlatCoordinates();//未导出

        // if (flatCoordinates && flatCoordinates.length > 100) {
        //      //从数组中删除第一个坐标元素
        //     flatCoordinates.splice(0,geometry.getStride());
        //       //以坐标元素作为参数调用推送
        //     // Array.prototype.push.apply（flatCoordinates， co坐标）;
        //     //更新坐标调用更改（）
    ...
    for (var i = 1; i < array.length; i++) {
        ...
        var datatmp1=[];
        datatmp1.push(array[i]);
        //var info =datatmp1
        lineDrawList.appendCoordinate(datatmp1);
...
$.TimerDrawLineAdd = function (ars) {
    ...
    //  var datatmp=[];
    //  datatmp.push(ars);
    //  lineDrawList.appendCoordinate(datatmp);
